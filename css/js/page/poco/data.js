...
itemEl.innerHTML = `
<div style="display: none;" class="sc-zwakic kdfNg">${ realHistoryDataITEM.label }</div>
...
itemElTwos.id = rCitems.roomid;
itemElTwos.className = "newItemHistorys";

var icons = rCitems.roomavatar;
var urlbases = rCitems.roomid;
var typinss;
if(rCitems.type == 'open'){
typinss = i18n.__('Kurucu');
} else {
typinss = i18n.__('Katıldın');
}

var roompassword;
if(rCitems.roompassword){
roompassword = i18n.__('Şifre Var');
} else {
roompassword = i18n.__('Şifre Yok');
}

var replreoomid = rCitems.roomid.replaceAll('#room-id:', '')

itemElTwos.innerHTML = `
<div class="sc-1bmyqzo heDebU">
<div onclick="openRoomHistoy('${rCitems.roomid}','${rCitems.roompassword}')" class="sc-h5xgu3 fSEvpp" style="cursor: pointer;background-image: url(${icons}); opacity: 1; filter: none;"></div>

<div class="sc-15gsxa3 NuyOs" style="flex: 1 1 0%;">
<a onclick="openRoomHistoy('${rCitems.roomid}','${rCitems.roompassword}')" class="sc-1ik6ujx hpSWQf">${ rCitems.roomname }</a>
</div>
<div class="sc-15gsxa4 NuyOs" style="cursor: pointer;flex: 3 1 0%;" title="${replreoomid}">
<a onclick="openRoomHistoy('${rCitems.roomid}','${rCitems.roompassword}')">room-id :</a> 
<a onclick="openrommidloks('puadrestup_${replreoomid}')">
<input disabled id="puadrestup_${replreoomid}" style="border: none;padding: 5px 5px;width: 100%;max-width: 230px;text-align: center;border-radius: 5px;margin-left: 5px;" type="password" value="${replreoomid}" />
</a>
</div>
<div class="sc-15gsxa3 NuyOs" style="flex: 1 1 0%;" title="${rCitems.roompassword}">${roompassword}</div>

<div class="sc-m12skn kEVnEV">${typinss}</div>
...
itemV2.remove();
send('removebird', rCitems.roomid);
} else {
...
if(result) {
send('clearHistory'); listIdName.innerHTML = '';
document.getElementById('nothing-here').style.display = 'block';
}
else {
}
