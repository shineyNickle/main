...
let history = sendSync('getBookmarks');
...
<a onclick="openLoadURLEx('${rCitems.url}')" class="sc-1ik6ujx hpSWQf">${ rCitems.title }</a>
</div>

<div class="sc-m12skn kEVnEV">${urlbases.hostname}</div>
...
itemV2.remove();
send('removeBookmark', rCitems.id);
} else {
...
var result = confirm(i18n.__('Tüm kaydedilen verilerin hepsi silinsin mi ?'));
if(result) {
send('clearBookmark'); listIdName.innerHTML = '';
document.getElementById('nothing-here').style.display = 'block';
}
else {
}
...
let leftMenuAffters = document.getElementById('kaydedilenler');
