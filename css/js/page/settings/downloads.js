...
let history = sendSync('getDownloads');
...
var icons = 'assets://images/settings/minetype/file.png';

var minetypeextends = rCitems.getMimeType.split('/');
var minetypes = minetypeextends[0];


if(minetypes == 'image'){ var icons = 'assets://images/settings/minetype/image.png'; }
if(minetypes == 'video'){ var icons = 'assets://images/settings/minetype/video.png'; }
if(minetypes == 'audio'){ var icons = 'assets://images/settings/minetype/music.png'; }


var fileName = rCitems.title;
const mimeType = fileName.split('.').pop();
if(mimeType == 'rar'){ var icons = 'assets://images/settings/minetype/winrar.png'; }
if(mimeType == 'zip'){ var icons = 'assets://images/settings/minetype/winrar.png'; }
if(mimeType == 'gz'){ var icons = 'assets://images/settings/minetype/winrar.png'; }
if(mimeType == 'pdf'){ var icons = 'assets://images/settings/minetype/pdf.png'; }
if(mimeType == 'torrent'){ var icons = 'assets://images/settings/minetype/torrent.png'; }
if(mimeType == 'exe'){ var icons = 'assets://images/settings/minetype/exe.png'; }
if(mimeType == 'txt'){ var icons = 'assets://images/settings/minetype/txt.png'; }
if(mimeType == 'sql'){ var icons = 'assets://images/settings/minetype/sql.png'; }
if(mimeType == 'js'){ var icons = 'assets://images/settings/minetype/js.png'; }


if(rCitems.status == 'wait'){ var vstatus = i18n.__('Bekliyor'); }
if(rCitems.status == 'completed'){ var vstatus = i18n.__('Tamamlandı'); }
if(rCitems.status == 'failed'){ var vstatus = i18n.__('İndirme Başarısız'); }
if(rCitems.status == 'cancel-download'){ var vstatus = i18n.__('İptal Edildi'); }


if (fs.existsSync(rCitems.url)) {
} else {
if(rCitems.status == 'completed'){
vstatus = i18n.__('Silindi');
rCitems.status = 'delete';
rCitems.url = '';
}
}
...
<div class="sc-1bmyqzo heDebU ${rCitems.status}" id="two_${rCitems.id}">
...
<a id="shellinkso" style="cursor: pointer;" onclick="getShellGo('${ rCitems.url }')" class="sc-1ik6ujx hpSWQf">${ rCitems.title }</a>
<br>
<div class="newwarulrkll"><a onclick="openLoadURLEx('${rCitems.urlreal}')" class="sc-1ik6ujx hpSWQf">${rCitems.urlreal}</a></div>
</div>

<div class="sc-m12skn kEVnEV statuss">${vstatus}</div>
<div class="sc-1uh5z3d kYVdAJ"> ${timestampDate(rCitems.time)}</div>
<div id="oDelete" class="sc-m21a46 cBxkta"></div>
<div id="oDowloanCancel" class="sc-m21a46 cBxkta" style="display:none"></div>
</div>
`.trim();
...
if(e.target.id == 'oDowloanCancel') {
ipcRenderer.send('cancel-download-' + rCitems.id);
}

if(e.target.id == 'oDelete') {
itemV2.remove();
send('removeDownloads', rCitems.id);
} else {
...
var result = confirm(i18n.__('Tüm indirilen verilerin hepsi silinsin mi ?'));
if(result) {
send('clearDownloads'); listIdName.innerHTML = '';
document.getElementById('nothing-here').style.display = 'block';
}
else {
}
...
async function getShellGo (urls) { if(urls){ send('getShellGo', urls); } }
...
if(document.querySelector('#two_' + id)){
document.querySelector('#two_' + id + ' .progressbar > div').style.width = percentage + '%';
document.querySelector('#two_' + id + ' .statuss').innerHTML =  formatBytes(getReceivedBytes) + ' - ' + formatBytes(getTotalBytes);

document.querySelector('#two_' + id + ' #oDelete').style.display = 'none';
document.querySelector('#two_' + id + ' #oDowloanCancel').style.display = 'block';
}
});

ipcRenderer.on('completeDownload', async (e, id, loc) => {
document.querySelector('#two_' + id + ' .progressbar').style.display = "none";
//document.querySelector('#two_' + id + ' button').style.display = "none";
document.querySelector('#two_' + id + ' .statuss').innerHTML =  i18n.__('Tamamlandı');
var submitButton = document.querySelector('#' + id + ' #shellinkso');
submitButton.setAttribute('onclick', `getShellGo('${ loc }')`);

document.querySelector('#two_' + id + ' #oDelete').style.display = 'block';
document.querySelector('#two_' + id + ' #oDowloanCancel').style.display = 'none';
});

ipcRenderer.on('failedDownload', async (e, id) => {
//document.querySelector('#one_' + id).style.border = "1px red solid";
document.getElementById(id).classList.add('cancel-download'); 
document.querySelector('#two_' + id + ' .statuss').innerHTML = i18n.__('İndirme Başarısız');
document.querySelector('#two_' + id + ' .progressbar').style.display = "none";
...
let leftMenuAffters = document.getElementById('indirilenler');
